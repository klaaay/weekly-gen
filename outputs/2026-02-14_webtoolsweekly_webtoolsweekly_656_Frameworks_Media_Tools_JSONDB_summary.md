### [网站性能与核心网页指标监控 | DebugBear](https://www.debugbear.com/?utm_campaign=webtoolsweekly-2026b)

**原文标题**: [Web Performance And Core Web Vitals Monitoring | DebugBear](https://www.debugbear.com/?utm_campaign=webtoolsweekly-2026b)

DebugBear 是一个全面的网站性能监控平台，专注于帮助用户优化网页速度、提升用户体验和改善谷歌排名。

- 🚀 **识别慢速页面** – 通过真实用户分析或网站扫描，发现访客体验不佳的页面。
- 🔍 **诊断性能瓶颈** – 利用详细的技术报告修复性能问题。
...
- 📈 **行业基准对比** – 基于谷歌真实访客数据，比较与竞争对手的网站性能，查看长期趋势。
- 👍 **用户口碑** – 用户评价其提供高度可操作的信息、独特的调试功能，是监控核心网页指标和网站性能的可靠工具。

---

### [GitHub - hsuanyi-chou/shadcn-ui-expansions：基于shadcn-ui构建的更多组件。](https://github.com/hsuanyi-chou/shadcn-ui-expansions)

**原文标题**: [GitHub - hsuanyi-chou/shadcn-ui-expansions: More components built on top of shadcn-ui.](https://github.com/hsuanyi-chou/shadcn-ui-expansions)

这是一个基于 shadcn/ui 构建的开源项目，提供了更多实用的 React 组件，适用于个人和商业用途，采用 MIT 许可证。

- 🧩 **扩展组件库** – 在 shadcn/ui 基础上新增了多种实用组件，如多选器、加载按钮、无限滚动等
- 🛠️ **即拿即用** – 所有组件均可直接复制粘贴到项目中，并自由定制
- 🌐 **在线演示** – 提供演示网站供预览和体验组件效果
- 🔧 **技术栈** – 基于 React 和 Next.js 开发，使用 TypeScript 编写
- 🤝 **开源贡献** – 欢迎通过提交 Issue 或 Pull Request 来报告问题或新增功能
- 📄 **项目信息** – 已获得 1.9k 星标、83 个分支，由 Hsuan Yi Chou 创建维护
...
### [Bundui：Tailwind CSS 与 shadcn/ui 组件、UI 区块、示例及更多资源。](https://bundui.io/)
...
Reshaped 是一个专业的设计系统，提供完全对齐的 React 和 Figma 组件库，旨在帮助团队快速构建美观的产品或启动自己的设计系统。

- 🚀 **快速启动开发**：通过原子化组件方法，显著提升开发速度，支持与 Next.js、Vite 等框架轻松集成。
- 🎨 **设计与代码完全同步**：Figma 库与 React 组件 100% 匹配，使用最新功能（如 Figma 变量），确保设计一致性。
- 🌙 **自动深色模式**：基于语义化设计令牌，自动应用主题和色彩模式，无需重复设计或编写样式。
- 📱 **响应式样式支持**：提供响应式语法，无需编写媒体查询即可根据视口大小调整样式。
- ♿ **全面无障碍访问**：所有组件均符合 WCAG 和 WAI-ARIA 标准，确保良好的可访问性。
- 🔧 **灵活的主题定制**：提供主题 CLI 工具，支持轻松创建新主题、添加设计令牌，并与 TailwindCSS 等工具集成。
- 📚 **详细文档与最佳实践**：文档包含交互式示例和上下文提示，帮助用户深入理解系统的每个细节。
- 💬 **积极的团队支持**：开发团队积极响应用户反馈，持续改进产品，提供可靠的长期支持。
...
FireGEO 是一个基于 Next.js 15 的开源 SaaS 启动模板，集成了品牌监控、用户认证、计费系统和 AI 聊天功能，支持快速部署和零配置启动。

- 🚀 **快速启动**：提供一键式设置脚本，自动安装依赖、配置数据库并准备开发环境。
- 🛠️ **技术栈丰富**：采用 Next.js 15、React 19、TypeScript，结合 PostgreSQL、Drizzle ORM、Better Auth 和 Autumn 计费。
- 🔑 **多服务集成**：支持 Firecrawl 网页抓取、多种 AI 提供商（OpenAI、Anthropic 等）、Resend 邮件服务和 Stripe 支付。
- 📁 **结构化项目**：清晰的目录组织，包含 API 路由、认证页面、仪表盘、聊天界面和品牌监控工具。
- ⚙️ **灵活配置**：提供手动和自动两种设置方式，详细的环境变量说明和常见问题解决方案。
- 🌐 **生产就绪**：支持 Vercel 部署，包含数据库迁移、环境配置和故障排除指南。
- 📜 **开源许可**：基于 MIT 许可证，鼓励社区贡献和自定义扩展。

---

### [GitHub - webdevcody/ppai-next-starter](https://github.com/webdevcody/ppai-next-starter)
...
### [PasteResize — 免费在线图片调整与编辑器](https://pasteresize.online/)
...
### [获取失败](https://recs.page/web-tools-weekly?ref_code=e0ae613693&email=<<subscriber@example.com>>)

**原文标题**: [Failed to retrieve](https://recs.page/web-tools-weekly?ref_code=e0ae613693&email=<<subscriber@example.com>>)
...
### [Promptefy](https://www.promptefy.online/)

**原文标题**: [Promptefy](https://www.promptefy.online/)
...
image-js 是一个用于 JavaScript 的图像处理和操作库，提供完整的 API 文档和开发指南，采用 MIT 许可证，由 Zakodium 维护。

- 🖼️ 图像处理与操作：提供 JavaScript 中的图像处理和操作功能
- 📦 安装简便：可通过 npm 安装，命令为 `npm install image-js`
- 📚 完整文档：拥有完整的 API 文档和使用指南
- 🔧 开发支持：提供详细的开发文档
- ⚖️ 开源许可：采用 MIT 许可证
- 🌐 项目活跃：拥有 1k+ 星标、76 个分支和 137 个版本发布
- 👥 社区贡献：由 20 位贡献者维护，被 2.1k+ 项目使用

---

### [圆形裁剪图片 - 免费在线圆形图片裁剪工具](https://circlecropimage.net/)

**原文标题**: [Circle Crop Image - Free Online Circle Image Cropper](https://circlecropimage.net/)
...
### [可视化证明：您的用户界面行之有效 | Vizzly](https://vizzly.dev/?utm_source=webtoolsweekly&utm_medium=newsletter&utm_campaign=2026q1&utm_content=feb12-reviewui)
...
RegreSQL 是一个用于对 SQL 查询进行回归测试的工具，它通过运行 SQL 文件并与已知的预期结果进行比较，来检测查询结果或性能特征的变化。

- 🧪 **回归测试工具**：专为 SQL 查询设计，通过对比当前输出与基准结果来检测意外变更。
- 🗂️ **文件驱动**：自动发现项目中的 `.sql` 文件，支持多查询文件和通过注释命名查询。
- ⚙️ **参数化查询**：支持命名参数（如 `:id`）和位置参数（如 `$1`），参数值在独立的 YAML 计划文件中配置。
- 📸 **快照功能**：可捕获数据库模式和数据状态，确保测试的可重复性，并支持版本标记和差异比较。
- 🔄 **迁移测试**：支持在数据库迁移前后运行查询，以评估迁移对查询结果的影响。
- 📊 **性能基线**：可创建查询执行计划（EXPLAIN）的成本基线，用于检测查询性能回归。
- 🛠️ **丰富命令集**：提供 `discover`（发现）、`add`（添加）、`update`（更新基准）、`test`（运行测试）等核心命令。
- ⚡ **快速集成**：可通过 Homebrew 或 Go 安装，并需要 PostgreSQL 客户端工具以支持快照功能。
- 📁 **结构化输出**：测试结果支持多种格式（如控制台、JUnit、JSON），并生成清晰的目录结构存放配置、计划和预期结果。

---

### [Reinder - 数据库光标工具](https://www.tryreindeer.com/)

**原文标题**: [Reinder - Cursor for databases](https://www.tryreindeer.com/)
...
### [StackRender – 视觉优先的数据库迁移工具](https://www.stackrender.io/)
...
JSON Editor 是一个基于 JSON Schema 生成 HTML 表单的 JavaScript 库，支持 JSON Schema 版本 3 和 4，并可与多种 CSS 框架集成。目前处于维护模式，建议新项目考虑使用 Jedison。

- 📄 **基于 JSON Schema**：根据 JSON Schema 自动生成对应的 HTML 表单。
- 🎨 **主题与图标库支持**：内置多种 CSS 主题（如 Bootstrap、Spectre、Tailwind）和图标库（如 Font Awesome）。
- 🔌 **丰富的扩展集成**：支持 Ace Editor、SimpleMDE、Flatpickr 等多种第三方编辑器与组件。
- ⚙️ **灵活的配置选项**：提供大量选项来自定义表单行为，如验证、布局、依赖关系等。
- 🔗 **Schema 引用与依赖**：支持通过 `$ref` 引用外部或本地 Schema，并可通过 `watch` 和模板实现字段动态依赖。
- 📱 **响应式与交互功能**：支持表单验证、动态枚举、条件显示、数组拖拽排序等交互特性。
- 🌍 **多语言与自定义**：可自定义界面文本，支持创建自定义编辑器接口和验证规则。
- 📦 **易于安装与使用**：可通过 npm 或 CDN 快速引入，并提供大量示例与详细文档。

---

### [免费离线数据库架构设计器](https://relatedb.com/)
...
### [GitHub - liweiyi88/onedump：轻松数据库管理工具](https://github.com/liweiyi88/onedump)

**原文标题**: [GitHub - liweiyi88/onedump: Effortless database administration tool](https://github.com/liweiyi88/onedump)

Onedump 是一款数据库管理工具，专注于简化跨多种数据库和存储目标的备份与恢复任务，支持 MySQL 和 PostgreSQL，并提供本地文件系统、AWS S3、Google Drive、Dropbox 和 SFTP 等多种存储目的地。

- 🛠️ **功能丰富**：支持数据库备份与恢复、MySQL 原生转储、MySQL 二进制日志备份与恢复、慢查询日志解析、可恢复的并发 SFTP 文件传输以及 Slack 通知。
- 📦 **安装灵活**：提供二进制文件和 Docker 镜像两种安装方式，Docker 镜像已包含 `pg_dump` 和 `mysql-client` 等依赖。
- ⚙️ **配置多样**：可通过本地配置文件或直接从 AWS S3 存储桶加载配置，支持通过 SSH 连接远程数据库。
- 🔄 **运行模式**：支持单次执行和定时任务（通过 cron 表达式），并可控制最大并发作业数。
- 📝 **原生转储器**：内置 MySQL 原生转储器，适用于基本场景，也支持切换到 `mysqldump` 以使用更高级功能。
- 📊 **附加工具**：包含慢查询日志解析器、MySQL 二进制日志备份到 S3 以及从二进制日志恢复数据的功能。
- 🤝 **开源贡献**：项目基于 MIT 许可证开源，欢迎开发者参与贡献。
...
总结失败：Error code: 400 - {'error': {'message': "This model's maximum context length is 131072 tokens. However, you requested 138948 tokens (130948 in the messages, 8000 in the completion). Please reduce the length of the messages or completion.", 'type': 'invalid_request_error', 'param': None, 'code': 'invalid_request_error'}}
...
### [获取失败](https://recs.page/web-tools-weekly?ref_code=bc5d6b37cb&email=<<subscriber@example.com>>)

**原文标题**: [Failed to retrieve](https://recs.page/web-tools-weekly?ref_code=bc5d6b37cb&email=<<subscriber@example.com>>)
...
SATIS UI 是一个不断增长的 React 组件库，旨在通过提供具备 Awwwards 级别动画效果、基于现代技术栈（如 Next.js、React、TypeScript、Tailwind CSS 等）构建的生产级组件，帮助开发者快速构建精美的用户界面。它强调开箱即用，支持直接复制、粘贴和部署，并提供 CLI 和手动安装两种方式。

- 🚀 **加速开发流程**：提供不断增长的生产级 React 组件集合，带有精美的动画效果，支持直接复制、粘贴和部署。
- 🛠️ **基于现代技术栈**：使用 Next.js、React、TypeScript、Tailwind CSS、GSAP、Motion 和 Shadcn 等流行库和框架构建。
- 🎨 **丰富的组件示例**：包括轮播（如缩略图进度轮播、故事轮播）、卡片（如焦点卡片、个人资料卡片、博客卡片）、文本效果（如颜色循环文本、翻转文字交换）、按钮（如微扩展按钮、幕布按钮、变形按钮）以及其他杂项组件（如扩展网格、邻近图像行）。
- ⚙️ **灵活的安装方式**：提供通过最新 Shadcn CLI 快速安装或手动复制粘贴两种安装方法，方便不同偏好。
- 🌟 **即将推出高级版本**：SATIS UI PRO 即将发布，提供更全面的工具和资源。
- 👨💻 **由设计工程师打造**：由 Satish Kumar 创建，专注于将高保真设计转化为像素级完美的生产级应用，涵盖 UI/UX 设计、前端架构和全栈开发。
...
Tailwindadmin 是一款基于 React 和 Tailwind CSS 构建的现代化 Shadcn 仪表板模板，提供丰富的 UI 组件、模块和即用页面，帮助开发者快速构建功能丰富的仪表板或管理面板。该模板支持多版本框架，包含专业版功能，并附带详细文档与支持。

- 🆓 **免费提供** – 提供免费版本下载，包含基础仪表板、UI 组件和页面模板，适合快速启动项目。
- 🛠️ **技术栈明确** – 基于 React v19.2.0、Shadcn v3.2.1、Tailwind CSS v4.1.13 和 TypeScript v5.0.0 构建，确保现代开发体验。
- 📦 **功能丰富** – 包含 4+ 仪表板、45+ 页面模板、100+ UI 元素、15+ 图表及 8+ 认证页面，覆盖多种应用场景。
- 🎨 **高度可定制** – 支持多布局（LTR/RTL）、多配色方案，并包含 Figma 设计文件，便于品牌定制。
- 🤖 **AI 辅助工具** – 提供 AI 表单、表格和图表生成器，可快速创建带验证的现代化组件。
- 🌐 **多框架版本** – 除 React 版外，还提供 Next.js、Vue.js、Angular 和 TanStack Start 版本，适应不同技术栈需求。
- 📚 **文档与支持** – 提供详细文档、社区支持和专业版邮件支持，保障开发顺利进行。
- ⭐ **用户认可** – 已获 1067+ 开发者与机构信任，产品评分达 4.96/5，累计下载量超过 95 万次。
- 🚀 **专业版升级** – 专业版包含更多高级功能（如 Supabase/NextAuth/Firebase 集成）、更多演示变体和技术支持，满足企业级需求。
...
### [未找到标题](https://x.com/TheCineprism/status/2021486991540617721)
...
### [Atomix - 互动元素周期表](https://independent-coder.github.io/Atomix/)
