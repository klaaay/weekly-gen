...
def parse_latest_issue_info(soup):
    """从归档页提取最新一期 Frontend Focus 信息，兼容新版 issue-card 结构。"""
    ...
    issue_link = soup.find(
        "a",
        href=re.compile(r"^/?issues/\d+$"),
        string=re.compile(r"Issue #\d+"),
    )
    if not issue_link or "href" not in issue_link.attrs:
        return None, None

    return issue_link.get_text(" ", strip=True), issue_link["href"].lstrip("/")


def collect_issue_article_links(soup):
    """收集 Frontend Focus 期刊正文链接，兼容直接外链与旧版跳转链接。"""
    ...
    for link in candidate_links:
        ...
        path = parsed.path
        is_frontendfoc_redirect = domain in {"frontendfoc.us", "www.frontendfoc.us"} and path.startswith("/link")
        is_relative_redirect = href.startswith("/link")
        if domain in {"frontendfoc.us", "www.frontendfoc.us"} and not is_frontendfoc_redirect:
            continue
...
def scrape_frontendfoc():
    ...
    if response.status_code == 200:
        ...
        if issue_title and link_url:
            ...
            if check_and_skip_if_same_issue(script_name, link_url, "https://frontendfoc.us"):
                return
