...
def scrape_leadershipintech():
    outputs_dir = "outputs"
    if not os.path.exists(outputs_dir):
        os.makedirs(outputs_dir)
        print(f"创建输出目录：{outputs_dir}")

    base_url = "https://leadershipintech.com"
    url = f"{base_url}/"
    ...
    soup = BeautifulSoup(response.text, "html.parser")
    site_title = "Leadership in Tech"
    safe_title = "leadershipintech"
    ...
    extract_links_and_summarize(
        ...
        on_complete=create_telegram_completion_callback("Leadership in Tech"),
    )


if __name__ == "__main__":
    scrape_leadershipintech()
