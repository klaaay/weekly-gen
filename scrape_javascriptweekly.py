    ...
    if response.status_code == 200:
        ...
        if issue_link and 'href' in issue_link.attrs:
            ...
            if link_response.status_code == 200:
                ...
                # Find all <a> tags with href starting with "https://javascriptweekly.com/link"
                javascriptweekly_links = link_soup.find_all('a', href=re.compile(r'^https://javascriptweekly\.com/link'))
                
                # If no https links found, try relative links
                if not javascriptweekly_links:
                    # Find links that start with "/link"
                    javascriptweekly_links = link_soup.find_all('a', href=re.compile(r'^/link'))
                
                if javascriptweekly_links:
                    print(f"\nFound {len(javascriptweekly_links)} links with javascriptweekly.com/link pattern:")
                    
                    # 记录成功获取的文章数
                    processed_articles = 0
                    
                    for link in javascriptweekly_links:
                        if 'href' in link.attrs:
                            ...
                            # 避免重复处理相同的链接
                            if processed_articles > 0 and link_url in [l['href'] for l in javascriptweekly_links[:processed_articles]]:
                                continue
                    ...
                    print("\nNo links with javascriptweekly.com/link pattern found on the page.")
            else:
